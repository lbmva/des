...
const calculatePC2 = (c0, d0) => {
  return keyWrapping(c0, d0).map((x, xi) => {
    const c = x[`c${xi + 1}`]
    const d = x[`d${xi + 1}`]
    return permutation(`${c}${d}`, matrixPC2)
  })
}
...
const Ciphering = (l, r, K) => {
  ...
  K.forEach((k, ki) => {
    ...
    const PB = permutation(B, matrixPBox)
...
const DESEncrypt = (plaintext, key) => {
  ...
  //Генерация ключа
  const K = hexToBinary(key) // конвертация 16-го ключа в 2-ый
  const Kplus = permutation(K, matrixPC1)
  ...
  const Cipher = permutation(Encipher.join(''), invertPermutateTable)
